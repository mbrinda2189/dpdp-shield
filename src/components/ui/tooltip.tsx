...
// Lightweight tooltip replacements (no Radix dependency)
...
const TooltipContent = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement> & { side?: string; align?: string; hidden?: boolean; sideOffset?: number }>(
  ({ className, hidden, ...props }, ref) => (
    hidden ? null : <div ref={ref} className={cn("z-50 overflow-hidden rounded-md border bg-popover px-3 py-1.5 text-sm text-popover-foreground shadow-md", className)} {...props} />
  )
);
